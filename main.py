...
if __name__ == '__main__':
    ...
    settings = Settings(
        name="pos_test", save_path="saved_models/test", loss="ctc", device=torch.device("cuda:0"),
        report_progress_every=100, epochs=10, tau=2
    )

    labeller = SequenceLabeller(settings=settings)
    labeller = labeller.fit(train_data=train_data)
